...
extension FlashcardsView {
    ...
    // MARK: - Create / Update (via Standard-List-Picker)

    /// **User-Revision 2026-04-22**: Create-Flow nutzt jetzt den
    /// Standard-`FlashcardStackComposerSheet`. Wir bekommen nur die
    /// gewählten Listen-IDs zurück und bauen daraus einen neuen
    /// PersonalDeck. Name wird automatisch aus den Listennamen
    /// abgeleitet (gekürzt; Umbenennen geht via ContextMenu).
    ...
    /// Edit-Flow: bestehenden PersonalDeck mit neuen Listen aktualisieren.
    /// Listen-Änderung regeneriert `cardOrder` + setzt Fortschritt zurück
    /// (wir können nicht sauber mappen, wenn Karten wegfallen/dazukommen).
    /// Name bleibt erhalten — falls der User ihn selbst geändert hat, will
    /// er ihn nicht verlieren, nur weil er die Listen getauscht hat.
    func updatePersonalDeck(_ deck: PersonalDeck, withSelectedListIDs selectedIDs: Set<UUID>) {
        guard !selectedIDs.isEmpty else { return }
        let oldLists = Set(deck.sourceListIDs)
        let newLists = selectedIDs
    ...
    /// Auto-Name aus den gewählten Listen: bis zu 3 Listennamen mit „ · "
    /// verbunden, danach „ +…". Fallback „Mein Stapel", wenn leer.
