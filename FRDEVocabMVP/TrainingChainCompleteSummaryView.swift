...
/// **Konfetti-Burst-Overlay** (Stufe 5, 2026-05-02). Canvas + TimelineView-
/// gestützte Particle-Animation. ~60 Partikel fallen mit Drift, Rotation
/// und Fade-Out über 2.5 s. Niedrig-frequent (30 Hz Refresh), keine
/// Hot-Path-Performance-Issue.
private struct ConfettiBurst: View {
    let startDate: Date

    private static let particleCount: Int = 60
    private static let lifetime: Double = 2.5
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: 1.0 / 30.0)) { context in
            Canvas { ctx, size in
                ...
                guard elapsed >= 0, elapsed <= Self.lifetime else { return }

                // Fade-Out in der letzten Sekunde des Lifetimes.
                let fadeStart = Self.lifetime - 1.0
